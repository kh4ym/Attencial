...
public class AttendanceController : ControllerBase
{
    ...
    public async Task<IActionResult> SimulateScan(int sessionId)
    {
        ...
        {
            return BadRequest(new ApiResponse<string> { Success = false, Message = "No students are enrolled in this course." });
        }

        var markedStudentIds = await _context.AttendanceRecords
            .Where(ar => ar.SessionId == sessionId)
            .Select(ar => ar.StudentId)
            .ToListAsync();

        var remainingStudents = enrolledStudents
        ...
        {
            var rand = new Random();
            selectedStudent = remainingStudents[rand.Next(remainingStudents.Count)];
        }
        else
        {
            var rand = new Random();
            selectedStudent = enrolledStudents[rand.Next(enrolledStudents.Count)];
            
            var existingRecord = await _context.AttendanceRecords
                .FirstOrDefaultAsync(ar => ar.SessionId == sessionId && ar.StudentId == selectedStudent.Id);
        ...
        var randConfidence = new Random().NextDouble() * 15.0 + 82.0; // 82% to 97%
        var record = new AttendanceRecord
        {
            SessionId = sessionId,
            StudentId = selectedStudent.Id,
            Confidence = (float)randConfidence,
            DeviceId = "simulated-device-" + Guid.NewGuid().ToString().Substring(0, 8),
        ...
        {
            Success = true,
            Message = "Simulated scan recorded.",
            Data = new
            {
                StudentId = selectedStudent.Id,
